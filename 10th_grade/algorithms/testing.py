    ...
    def test_search(self):
        for test_tree in tests:
            ...
            for node in desired_nodes:
                ...
                self.assertTrue(found_node is not None and found_node.value == node)
    
    def test_add(self):
        for test_tree in tests:
            new_node_value = randint(-1000, 1000)
            test_tree.add(new_node_value)
            self.assertTrue(test_tree.correct() and test_tree.search(new_node_value) is not None)
    ...
    def test_delete(self):
        for test_tree in tests:
            ...
            for del_node in deletion_nodes:
                ...
                self.assertTrue(test_tree.correct() and test_tree.search(del_node) is None)
